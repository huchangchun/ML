...
def calcWs(alphas,dataArr,classLabels):
    ...
    return w
def showClassifer(dataMatIn, labelMat, w, b,alphas):
    data_plus =[]
    data_minus = []
    ...
    for i ,alpha in enumerate(alphas):
        if abs(alpha) > 0:
            x, y = dataMatIn[i]
            plt.scatter([x],[y], s=150,c='none', alpha=0.7, linewidths=1.5, edgecolors='green')
...
def testRbf(k1=0.1):
    ...
    showClassifer(dataArr, labelMat, w, b,alphas)
    ...
    errorCount = 0
    for i in range(m):
        kernelEval = kernelTrans(sVs,datMat[i,:],('rbf', k1))  #获取隐射后的向量内积<xi,x>
        predict=kernelEval.T * multiply(labelSV,alphas[svInd]) + b  #y = alpha*yi*<xi,x> + b
        if sign(predict)!=sign(labelArr[i]): errorCount += 1  #sign取数值符号
