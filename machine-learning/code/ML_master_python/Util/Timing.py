...
class Timing:
    ...
    def timeit(cls, level=0, func_name = None, cls_name = None, prefix="[Method]"):
        @wrapt.decorator
        def wrapper(func, instance, args, kwargs):#注意前面没有星号。在装饰器内部调用原函数时才带星号,instance参数时必须的。
            if not cls.enabled:
        ...
        if cls.timings:
            for key in sorted(cls.timings.keys()):
                ...
                if level >= timing_info["level"]:
                    print("{:<42s}:{:12.7} s (Call Time:{:6d})".format(key, timing_info["timing"], timing_info["call_time"]))
        print("-" * 110)
