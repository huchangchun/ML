...
class CvDBase(ClassifierBase):
    ...
    def __init__(self, whether_continuous=None, max_depth=None, node=None, **kwargs):
        ...
        self.prune_alpha 
        self.y_transformer 类别
        self.wheter_continuous 是否连续
        """
        self.nodes = []
        self.layers = []
        self.roots = []
    ...
    def feed_data(self, x, continuous_rate=0.2):
        ...
        if self.whether_continuous is None:
            self.whether_continuous = np.array([len(feat) >= int(continuous_rate * data_len) for feat in self.feature_sets])
        else:
    ...
    def prune(self, x_cv, y_cv, weights):
        ...
        if self.root.is_cart:
            if x_cv is not None and y_cv is not None:
                self._cart_prune()
                arg = np.argmax([CvDBase.acc(y_cv, tree.predict(x_cv), weights) for tree in self.roots])
